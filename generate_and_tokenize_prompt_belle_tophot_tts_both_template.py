...
def generate_and_tokenize_prompt(data_point, tokenizer=None, max_seq_length=-1, return_prompt=True):
    ...
    if not g_first_time_asserted:
        ...
        # ref: https://github.com/LianjiaTech/BELLE/blob/main/train/FAQ.md
    ...
    # 1. truncate tags
    truncated_tags = data_point['content']
    truncated_tags = truncated_tags[:2000] if isinstance(truncated_tags, str) else ' '

    if data_point['tt'] is not None and isinstance(data_point['tt'], str):
        prompt_part = f'你是小红书博主，需要根据输入的内容，按照指定的标题模版类型创作爆款小红书标题。\n小红书博主：\n需要起标题的内容：[{truncated_tags}]。\n' \
                      f'符合以上内容的[{data_point["tt"]}]模版类型的爆款标题：['
    else:
...
if __name__ == '__main__':
    ...
    data_file = "/home/ubuntu/cloudfs/ghost_data/newred_redbook_link_download/data_pipline/hot_tt_train/" \
        "merged_newred_redbook_downloads_joined_content_cleanedup_processed_all_months_til040623_hot_tt_expanded_tt_val.pickle"# "../dataset/instruction/guanaco_non_chat_mini_52K-utf8.json" #"./sample/merge_sample.json"
