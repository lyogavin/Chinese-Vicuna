...
def evaluation_loop(model, eval_dataloader, accelerator, tokenizer):
    model.eval()
    eval_loss = 0
    #eval_preds = []
    #predictions = []
    ...
    eval_metrics = {"eval/eval_ppl": eval_ppl, "eval/eval_epoch_loss": eval_epoch_loss}

    logger.info("on_evaluate samples ...")
    sample_predictions, sample_references = [], []
    for inputs in ["你好,中国的首都在哪里？",
                   '你是小红书博主，需要根据输入的内容，创作爆款小红书标题。\n自媒体创作者：\n需要起标题的内容：[' \
                   '移动开发框架 开发者 ]\n爆款标题：[',
                   '你是小红书博主，需要根据输入的内容，创作爆款小红书标题。\n自媒体创作者：\n需要起标题的内容：[' \
        ...
        generation_output_str = tokenizer.decode(generation_output[0])
        logger.info(generation_output_str)
        sample_predictions.append(generation_output_str)
...
def main():
    ...
    df = pd.read_csv(args.data_path, usecols=['title', 'content'])
    ...
    if isinstance(val_size_items, float):
        logger.info(f"converting float val size: {val_size_items}")
    ...
    if args.debug_mode:
        logger.info(f"debug_mode, dumping some data")
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(accelerator.skip_first_batches(train_dataloader, num_batches=resume_step)):
            ...
            if global_step % args.evaluation_steps == 0:
                ...
                if best_metric is None or eval_metrics["eval/eval_ppl"] < best_metric:
                    best_metric = eval_metrics["eval/eval_ppl"]
        ...
        if args.push_to_hub and epoch <= args.num_train_epochs - 1:
            ...
            eval_metrics = evaluation_loop(
                model, eval_dataloader, accelerator, tokenizer
            )
            if args.with_tracking:
                logger.info(f"Step {global_step} eval metrics: {eval_metrics}")
                accelerator.log(eval_metrics, step=global_step)
